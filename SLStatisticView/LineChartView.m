...
@interface LineChartView()<LinechartBasicViewDelegate>
{
    CGFloat xstepdefault;
    CGFloat myW;
    CGFloat myH;
}
@property (nonatomic, weak) LinechartBasicView* mystatisticsView;
@end
...
+(instancetype) builtNewstatisticViewWithFrame:(CGRect) rect statisticset:(statisticsSet *) set{
    LineChartView *newscr = [[self alloc]initWithFrame:rect];
    set.myW = rect.size.width;
    set.myH = rect.size.height;
    LinechartBasicView *staview = [LinechartBasicView buitStatisticsWithstatisticsSet:set];
...
-(void)setDataXarray:(NSMutableArray *) xarray Yarray:(NSMutableArray *) yarray{
    ...
    self.mystatisticsView.linechatset = self.linechatset;
